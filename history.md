...
## 2026-04-10: 사운드 효과 추가 + 게임 저장/복원 버그 수정

### 작업 개요
Web Audio API 기반 프로시저럴 사운드 효과 시스템 추가 및 브라우저 재시작 시 항상 레벨1부터 시작되는 버그 수정

### 수행 작업

#### 1. 사운드 효과 시스템 (src/audio/SoundManager.ts - 신규)
- Web Audio API OscillatorNode + GainNode를 사용한 프로시저럴 사운드 생성
- 외부 오디오 파일 불필요 (런타임 의존성 없음)
- 사운드 목록:
  | 이벤트 | 사운드 | 설명 |
  |--------|--------|------|
  | 매칭 성공 | 상승 2음 | C5→E5 빠른 상승음 |
  | 매칭 실패 | 저음 범프 | 짧은 저주파 삼각파 |
  | 버튼 클릭 | 클릭음 | 800Hz 짧은 사인파 |
  | 스테이지 클리어 | 승리 팡파레 | C5-E5-G5-C6 아르페지오 |
  | 게임 오버 | 하강 슬픈 톤 | A4-F#4-E4-C4 하강 |
  | 타이머 경고 (≤5초) | 긴급 비프 | 880Hz 짧은 스퀘어파 |
  | 힌트 사용 | 스파클 | 고주파 상승 시머 |
- 뮤트 상태 localStorage 영속 저장
- AudioContext lazy 초기화 (브라우저 사용자 제스처 요구사항 준수)

#### 2. 뮤트 버튼 UI
- **Renderer.ts**: 플레이 화면 HUD에 스피커 아이콘 (힌트 버튼 왼쪽)
- **InputHandler.ts**: `MUTE_TOGGLE` 커맨드 타입 추가, 히트 테스트
- **App.ts**: 뮤트 토글 커맨드 처리, RenderState에 `muted` 필드 추가

#### 3. 게임 저장/복원 버그 수정
- **원인**: `App.init()`에서 `hasSavedGame()` 확인 후 실제 `loadGame()`을 호출하지 않았음
- **증상**: 브라우저 재시작 시 항상 레벨1(IDLE 상태)에서 시작
- **수정**: `init()`에서 저장된 게임이 있으면 자동으로 `loadGame()` 호출하여 이전 상태 복원

### 변경 파일
| 파일 | 변경 |
|------|------|
| `src/audio/SoundManager.ts` | 신규 - 프로시저럴 사운드 매니저 |
| `src/App.ts` | SoundManager 통합, 자동 로드 버그 수정 |
| `src/ui/Renderer.ts` | 뮤트 아이콘 렌더링, RenderState.muted 추가 |
| `src/ui/InputHandler.ts` | MUTE_TOGGLE 커맨드, 뮤트 버튼 히트 테스트 |
